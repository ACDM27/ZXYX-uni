<template>
  <view class="form-container">
    <view class="form-content">
      ...
      <view class="form-item">
        <text class="label required">成果类别</text>
        <picker 
          class="picker" 
          mode="selector" 
          :range="achievementCategories" 
          @change="handleCategoryChange"
          :value="categoryIndex"
        >
          <view class="picker-text" :class="{ empty: !formData.category }">
            {{ formData.category || '请选择成果类别' }}
          </view>
        </picker>
      </view>
      ...
      <view class="form-item">
        <text class="label required">成果级别</text>
        <picker 
          class="picker" 
          mode="selector" 
          :range="achievementLevels" 
          @change="handleLevelChange"
          :value="levelIndex"
        >
          <view class="picker-text" :class="{ empty: !formData.level }">
            {{ formData.level || '请选择成果级别' }}
          </view>
        </picker>
      </view>
...
export default {
  data() {
    return {
      ...
      levelIndex: -1,
      awardLevelIndex: -1,
      achievementCategories: ['创新创业', '技能证书', '社会实践', '学科竞赛', '其他'],
      achievementLevels: ['国际级', '国家级', '省级', '市级', '校级'],
  ...
  methods: {
    handleCategoryChange(e) {
      ...
      this.formData.category = this.achievementCategories[index]
    },
    handleLevelChange(e) {
      const index = e.detail.value
      this.levelIndex = index
      this.formData.level = this.achievementLevels[index]
    },
